    ...
    _.extend(DateRangePicker.prototype, {
        ...
        buildPresetsList: function(){
            ...
            lis = _(presets).map(function(val, key){
                return '<li data-startdate="' + val.startDate + '" data-enddate="' + (val.endDate ||
                    val.startDate) +'" data-time="' + ((val.specifyTime) ? 'true' : 'false') + '">' + key + '</li>';
            });

            return '<div class="presets-wrapper"><div class="close"><a class="' +
                _.escape(this.closeButtonCssClass) + '"></a></div><nav class="presets"><h1>Quick Select</h1><ul>' +
                lis.join('') + '</ul></nav></div>';
        },
