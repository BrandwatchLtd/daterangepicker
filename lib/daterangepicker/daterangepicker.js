...
(function(root, factory){
    ...
    */
    ...
    function Calendar(options){
        ...
        this.showUtcOffset = options.showUtcOffset;
        ...
        $el.on('click', 'span.openUtcControls', _.bind(this.onOpenUtcControls, this));
        $el.on('change blur', '.utcOffset', _.bind(this.onUtcChanged, this));
    }

    function buildUtcControlsHtml(utcOffset){
        return '<div class="utcControls">' +
                    '<p>' +
                    '<label for="utcOffset">UTC Offset</label>' +
                    '<select class="utcOffset" name="utcOffset">' +
                    _.range(-12,13).map(function(i){
                        return '<option value="' + i + '"' +
                            (i === utcOffset ? ' selected>' :'>') +
                            i + '</option>'; }) +
                    '</select>' +
                    '</p>' +
                '</div>';
    ...
    function buildCalendarHtml(data){
        var rowHtml = _(data.rows).map(function(row){
            ...
            utcInput = !data.showUtcOffset ? '' : '<span class="icon-globe openUtcControls"></span>';
    ...
    _.extend(Calendar.prototype, {
        _getTimezoneUtcOffset: function(){
            var now = moment(),
                utc = moment.utc();

            return now.hours() - utc.hours();
        },

        _getUserUtcOffset: function(){
            return this._selectedUtcOffset || this._getTimezoneUtcOffset();
        },
        ...
        render: function(){
            var rowStartDate = this._getStartDate().subtract('days', 1),
                ...
                data = {
                    showUtcOffset: this.showUtcOffset,
        ...
        destroy: function(){
            ...
            $el.off('click', 'span.openUtcControls');
            $el.off('change blur', '.utcOffset');

            $el.remove();
        },

        getUtcOffset: function(){
            return this.showUtcOffset ? this._getUserUtcOffset() : this._getTimezoneUtcOffset();
        },

        onOpenUtcControls: function(e){
            e.stopPropagation();

            this.showUtcControls($(e.target));
        },

        onUtcChanged: function(e){
            e.stopPropagation();

            this._selectedUtcOffset = parseInt(this.$el.find('.utcOffset').val(), 10);

            this.updateSelectedDate(this.selectedDate);

            this.hideUtcControls();
        },

        showUtcControls: function($target){
            var $utcControls = this.$utcControls;

            if(!$utcControls){
                $utcControls = this.$utcControls = $(buildUtcControlsHtml(this.getUtcOffset())).hide();
            }

            $utcControls.prependTo(this.$el).show().css({
                top: 0,
                right: $target.outerWidth()
            });
        },

        hideUtcControls: function(){
            if(!this.$utcControls){
                return;
            }

            this.$utcControls.hide();
        },
        ...
        updateSelectedDate: function(date, options){
            ...
            var newDate = this.selectedDate = moment(date),
                ...
                buildArgs = _.bind(function(){
                    var args = { date: date },
                        offset;

                    if(this.showUtcOffset){
                        offset = this._getUserUtcOffset();
                        args.utcOffset = _.isNumber(offset) ? offset : 0;
                    }
            ...
            if(!silent){
                this.trigger('onDateSelected', buildArgs());
            }
        },
    ...
    _.extend(DateRangePicker.prototype, {
        ...
        _makeDateSelectionArgs: function(startDate, endDate){
            var args = {
                startDate: {
                    date: startDate
                },
            ...
            if(this.startCalendar.showUtcOffset){
                args.startDate.utcOffset = this.startCalendar.getUtcOffset();
            }

            if(this.endCalendar && this.endCalendar.showUtcOffset){
                args.endDate.utcOffset = this.endCalendar.getUtcOffset();
            }
        ...
        buildPresetsList: function(){
            ...
            return '<div class="presets-wrapper"><div class="close">' +
                    '<a class="' + _.escape(this.closeButtonCssClass) + '"></a></div>' +
                    '<nav class="presets"><h1>Quick Select</h1><ul>'+ lis.join('') + '</ul></nav></div>';
        },
        ...
        render: function(){
            ...
            this.$el.addClass('picker').append(presetsList).append(calendarWrapper);
        },
        ...
        hide: function(){
            ...
            this.startCalendar.hideUtcControls();

            if(this.endCalendar){
                this.endCalendar.hideUtcControls();
            }
        ...
        onStartDateSelected: function(args){
            ...
            this.trigger('startDateSelected', this._makeDateSelectionArgs(startDate, endDate));
        },
        ...
        onEndDateSelected: function(args){
            ...
            this.trigger('endDateSelected', this._makeDateSelectionArgs(startDate, endDate));
        },
        ...
        onPresetClick: function(e){
            ...
            this.trigger('presetSelected', this._makeDateSelectionArgs(startDate, endDate));
        },
        ...
        return this.each(function() {
            var $self = $(this),
                ...
                formatter = options.dateFormatter || function(startDate, endDate){
                    return startDate.date.format('DD MMM YYYY') +
                            ' - ' +
