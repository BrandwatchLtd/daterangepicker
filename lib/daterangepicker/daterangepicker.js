    ...
    */
    ...
    '<thead>' +
        '<tr>' +
            ...
            '<th colspan="5" style="width: auto">{{monthTitle}}</th>' +
    ...
    _.extend(Calendar.prototype, {
        render: function(){
            var rowStartDate = this.startDate.subtract('days', 1),
                ...
                data = {
                    monthTitle: this.selectedMonth.format('DD M'),
                    rows: _.map(_.range(1,7), function(weekIdx){
                        return {
                            columns: _.map(_.range(7), function(dayIdx){
                                var date = rowStartDate.clone().add('days', weekIdx * (dayIdx+1) );
    ...
    _.extend(DateRangePicker.prototype, {
        ...
        createCalendar: function(month, year, selectedDate, className){
            var cal = new Calendar(month, year, selectedDate, className);
            return cal;
        },
