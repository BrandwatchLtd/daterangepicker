...
(function(root, factory){
    ...
    */
    ...
    function buildTimePicker(calendar, presetTimes) {
        var $el, timePickerHtml = '';

        timePickerHtml = '<ul class="bw-time-picker">';

        presetTimes.forEach(function(time) {
            timePickerHtml += '<li data-time="' + time + '">' + time + '</li>';
        });
    ...
    function buildCalendarHtml(data){
        ...
        timePickerHtml = '<tr class="time-picker-row">' +
                            '<td colspan="7">' + data.timepickerLabel + ' <input type="text" name="time" value="' + NO_TIME_SELECTED + '" /></td>' +
                        '</tr>';
    ...
    _.extend(Calendar.prototype, {
        ...
        _getTime: function() {
            var time = this.$el.find('input[name="time"]').val();
        ...
        render: function(){
            var rowStartDate = this._getStartDate().subtract('days', 1),
                ...
                data = {
                    ...
                    timepickerLabel: this.timepickerLabel ? '<span class="calendar-timepicker-label">' + this.timepickerLabel + '</span>' : '',
        ...
        onTimePicked: function(e, time) {
            ...
            $el.find('input[name="time"]').val(time).trigger('change');

            $('.bw-time-picker').fadeOut(100);
        ...
        onTimeFieldClicked: function(e) {
            ...
            var calendar = this,
                ...
                $timePicker
                    .appendTo('body')
                    .css({
                        top: timePickerPosition.top + $timePickerField.height(),
        ...
        addMarker: function(date){
            var selectedClassName = 'selected',
                timeStr = NO_TIME_SELECTED,
                $timeField = this.$el.find('input[name="time"]');
    ...
    function DateRangePicker(options){
        ...
        if(options.timepicker){
            this.startCalendar.timepicker = buildTimePicker(this.startCalendar, getPresetTimes());
        }
        ...
        if (!options.singleDate){
            ...
            if(options.timepicker){
                this.endCalendar.timepicker = buildTimePicker(this.endCalendar, getPresetTimes());
            }
        }
