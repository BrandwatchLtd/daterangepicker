    ...
    */
    ...
    function Calendar(options){
        ...
        if (this.showTimepicker) {
            $el.on('click.timePicker', 'input[name="time"]', _.bind(this.onTimeFieldClicked, this));
            $el.on('blur.timePicker', 'input[name="time"]', _.bind(this.onTimeFieldBlur, this));
            $el.on('focus.timePicker', 'input[name="time"]', _.bind(this.onTimeFieldFocus, this));
            $el.on('change.timePicker', 'input[name="time"]', _.bind(this.onTimeChanged, this));
            $el.on('timePicked.timePicker', _.bind(this.onTimePicked, this));
        }
    }
    ...
    function TimePicker(options) {
        ...
        if(!options.calendar){
            throw new Error('TimePicker requires options.calendar to be set');
        }

        timePicker.presetTimes = DEFAULT_PRESET_TIMES;
        timePicker.openOnTimeFieldFocus = true;
        timePicker.closeOnTimeFieldBlur = true;

        timePicker.$el = $('<ul>')
            .addClass(TIME_PICKER_CLASS)
            .html(buildTimePickerList(timePicker.presetTimes));
    ...
    _.extend(TimePicker.prototype, {
        ...
        addMarker: function(time) {
            var classesToRemove = [SELECTED_CLASS_NAME, PREVIOUSLY_SELECTED_CLASS_NAME].join(' ');

            this.$el.children().removeClass(classesToRemove);

            this._findItemByTime(time).addClass(SELECTED_CLASS_NAME);
        },
        ...
        scrollToShowTime: function(time) {
            ...
            $selectedTime = timePicker._findItemByTime(nearestPresetTime);
    ...
    _.extend(Calendar.prototype, {
        ...
        onTimePicked: function(e, time) {
            var calendar = this,
                $timeField = calendar._getTimeField();
        ...
        onDayClicked: function(e){
            ...
            var target = $(e.target),
                ...
                time = TIME_REGEX.exec(this._getTime());

            if (Array.isArray(time)) {
                date = moment(date).hours(time[1]).minutes(time[2]).toString();
            }
        ...
        addMarker: function(date){
            ...
            if (timePicker) {
                timePicker.addMarker(date);
                this._getTimeField().val(timePicker.getValue(date));
            }
        },
        ...
        highlightCells: function(startDate, endDate){
            ...
            if(Math.abs(startDate.diff(endDate)) < MILLISECONDS_PER_DAY){
                // difference is less than a day so no highlight needed
                return;
            }
    ...
    function DateRangePicker(options){
        ...
        if(options.timePicker){
            this.startCalendar.timePicker = TimePicker.create({
        ...
        if (!options.singleDate){
            ...
            if(options.timePicker){
                this.endCalendar.timePicker = TimePicker.create({
