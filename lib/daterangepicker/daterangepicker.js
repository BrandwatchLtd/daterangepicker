...
(function(root, factory){
    ...
    */
    ...
    _.extend(Calendar.prototype, {
        ...
        _addMarkers: function(startCell, endCell){
            var startMarker = this.markers.startMarker,
                endMarker = this.markers.endMarker,
                selectedClassName = 'selected',
                addMarker = function($el, text){
                    var $span = $('<span class="marker">' + text + '</span>').css({
                        position: 'absolute',
                        top: '-0.7em',
                        left: 0,
                        right: 0
                    ...
                    $el.addClass('hasMarker')
                        .css({ position: 'relative' })
                        .append($span);
                },
                removeMarkers = function($el){
                    $el.find('.selected').removeClass(selectedClassName);
                    $el.find('.hasMarker').removeClass('hasMarker').children('.marker').remove();
                };

            removeMarkers(this.$el);

            if(startMarker){
                startCell.addClass(selectedClassName);

                if(startMarker.text){
                    addMarker(startCell, startMarker.text);
                }
            }

            if(endMarker){
                endCell.addClass(selectedClassName);

                if(endMarker.text){
                    addMarker(endCell, endMarker.text);
                }
            }
        },
        ...
        highlightCells: function(startDate, endDate){
            ...
            this._addMarkers(startCell, endCell);
    ...
    _.extend(DateRangePicker.prototype, {
        ...
        buildPresetsList: function(){
            ...
            return '<div class="presets-wrapper"><nav class="presets"><h1>Quick Select</h1><ul>' + lis.join('') + '</ul></nav></div>';
        },
        ...
        render: function(){
            var startCalendar = this.startCalendar,
                ...
                calenderWrapperInner = $('<div class="calendar-wrapper-inner"/>');
            ...
            calenderWrapperInner.append(startCalendar.$el).append(endCalendar.$el).appendTo(calendarWrapper);
